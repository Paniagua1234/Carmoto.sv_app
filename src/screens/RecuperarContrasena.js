...
export default function RecuperarContrasena({ navigation }) {
    ...
    // Estados para manejar datos y mostrar la interfaz1
    ...
    const handleUs = async () => {
        try {
            ...
            const response = await fetch(`${ip}/Carmoto.sv/api/services/public/cliente.php?action=verifUs`, {
                method: 'POST',
                body: formData,
            });
    
            // Lee la respuesta como texto y revisa su contenido
            const text = await response.text();
            console.log('Respuesta del servidor:', text);
    
            // Intenta analizar el texto como JSON solo si no comienza con un "<"
            if (text.startsWith('<')) {
                console.error('El servidor devolvió HTML en lugar de JSON. Verifica el servidor.');
                return;
            }
    
            // Analiza el JSON si la respuesta parece ser válida
            ...
            if (data.status && data.dataset) {
                ...
                setAlertMessage(data.error || 'Correo inexistente o error en la respuesta del servidor');
                setShowAlert(true);
            }
        } catch (error) {
